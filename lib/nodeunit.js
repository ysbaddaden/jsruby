// Hacked from http://github.com/caolan/nodeunit
// MIT-License.
...
var rubyTestEnv = function(start, options)
{
  var test = testEnv(start, options);

  test.rb_same = function(expected, rb_code, json)
  {
    if (typeof json == 'undefined')
    {
      Ruby.compile(rb_code, function(iseq)
      {
        test.strictEqual(iseq.run(), expected);
        test.done();
      });
    }
    else
    {
      iseq = new Ruby.YARVInstructionSequence(json);
      test.strictEqual(iseq.run(), expected);
      test.done();
    }
  };

  test.rb_nil = function(rb_code, json) {
    test.rb_same(Ruby.objects['nil'], rb_code, json);
  };

  test.rb_true = function(rb_code, json) {
    test.rb_same(Ruby.objects['true'], rb_code, json);
  };

  test.rb_false = function(rb_code, json) {
    test.rb_same(Ruby.objects['false'], rb_code, json);
  };
...
var testEnv = function(start, options){
    ...
    return {
        ...
        ok: wrapAssert('ok', 'ok'),
        fail: wrapAssert('fail', 'fail'),
        equal: wrapAssert('equal', 'equal'),
        notEqual: wrapAssert('notEqual', 'notEqual'),
        deepEqual: wrapAssert('deepEqual', 'deepEqual'),
        notDeepEqual: wrapAssert('notDeepEqual', 'notDeepEqual'),
        strictEqual: wrapAssert('strictEqual', 'strictEqual'),
        notStrictEqual: wrapAssert('notStrictEqual', 'notStrictEqual'),
